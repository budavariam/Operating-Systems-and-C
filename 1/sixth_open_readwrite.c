...
int main(int argc, char **argv)
{
  ...
  if (f < 0)
  {
    perror("Error at opening the file\n"); //open tudja triggerelni a perror-t
    exit(1);
  }
  ...
  while (read(f, &c, sizeof(c)))
  {
    ...
    //1. parameter the file descriptor (leíró)
    //2. parameter the address of variable, we read into
    //3. parameter the number of bytes we want to read in (byte-szám amit be szeretnénk olvasni)
